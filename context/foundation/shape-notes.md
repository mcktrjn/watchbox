...
checkpoint:
  ...
  gray_areas_resolved:
    - topic: "pain category"
      decision: "brak dostępu do całej kolekcji w terenie; brak jednego estetycznego miejsca na zdjęcia+dane; brak wiedzy o realnym noszeniu"
    - topic: "core insight"
      decision: "żadna apka nie łączy wszystkich funkcji; alternatywy nieestetyczne i niepełne; przyszły hardware-tracker podbije wartość"
    - topic: "primary persona"
      decision: "kolekcjoner-entuzjasta z wieloma zegarkami, w tym autor"
    - topic: "auth strategy"
      decision: "logowanie e-mail + hasło; płaski model ról; dane po stronie serwera"
    - topic: "watch entry in MVP"
      decision: "tylko nazwa + zdjęcie, bez pól technicznych; auto-uzupełnianie z bazy wiedzy poza MVP"
    - topic: "mvp scope timeline"
      decision: "~3 tygodnie po godzinach, zaakceptowane jako realne"
    - topic: "wear-session editing"
      decision: "FR-010 (edycja/usuwanie sesji) awansowany do must-have"
    - topic: "stats granularity"
      decision: "oba ujęcia: sumaryczny czas noszenia (godziny) + liczba sesji/dni"
  frs_drafted: 11
...
# Shape Notes — Watchbox

> Seed idea source: `idea-notes.md` (verbatim seed captured by /10x-shape).
> Sekcje body uporządkowane w kolejności schematu PRD (greenfield, 10 sekcji); sekcje
> spoza schematu (Timeline budget, Quality cross-check, Forward) oznaczone jako pomocnicze.
...
niepełne. Dodatkowo planowany w przyszłości hardware (tracker mocowany do zegarka,
zbierający dane o noszeniu automatycznie) podniesie wartość całego rozwiązania;
aplikacja jest pierwszym krokiem w tym kierunku. MVP celowo zaczyna od modułu
statystyk noszenia jako funkcjonalności domenowej, bo pełna baza wiedzy wymaga
pozyskania danych w skali przekraczającej zakres MVP.
...
- (Brak osobnego kryterium nice-to-have — wcześniejszy kandydat „edycja/usuwanie
  sesji noszenia wstecz" został w fazie 4 awansowany przez użytkownika do must-have
  jako FR-010.)
...
- FR-009: Użytkownik może zarejestrować sesję noszenia zegarka, wskazując datę oraz godzinę założenia i godzinę zdjęcia (HH:MM). Priority: must-have
  > Socrates: Kontrargument rozważony: „ręczne godziny to tarcie; użytkownik może nie logować, a od tego zależy cała wartość statystyk". Rozstrzygnięcie: zostaje — ręczne wpisywanie jest świadomym wyborem MVP; automatyczny pomiar to przyszły hardware-tracker. Ryzyko dyscypliny logowania → Open Questions.
...
- **Automatyczny pomiar noszenia (hardware-tracker)** — przyszły produkt sprzętowy; w
  MVP dane noszenia są wyłącznie ręczne.
...
2. **Ryzyko dyscypliny logowania (FR-009).** — cała wartość statystyk zależy od regularnego ręcznego wpisywania sesji; brak danych = puste wykresy. Mitygacja docelowa: hardware-tracker (poza MVP). Owner: użytkownik.

---

## (Pomocnicze — poza schematem PRD)

### Timeline budget

- MVP scope (logowanie + CRUD pudełka + rejestracja sesji + statystyki) szacowany na
  ~3 tygodnie pracy po godzinach. Mieści się w dyscyplinie krótkiego MVP — koszt
  surfaced i zaakceptowany jako realny.

### Quality cross-check

Wynik bramki jakości (soft gate) — wszystkie elementy obecne, brak luk:

- Access Control: present — logowanie e-mail+hasło, płaski model ról.
- Business Logic: present — jednozdaniowa reguła rankingu noszenia.
- Project artifacts: present — shape-notes.md z poprawnym checkpointem.
- Timeline-cost ack: present — mvp_weeks = 3, w granicach krótkiego MVP.
- Non-Goals: present — 7 jawnych wykluczeń.

Status: accepted.

### Forward: tech-stack

> Informacyjne — NIE część schematu PRD. Do przejęcia przez krok wyboru stacku po /10x-prd.

- Preferencja dostarczenia: aplikacja webowa jako PWA z bardzo dobrą responsywnością
  na urządzeniach mobilnych (główny scenariusz „cała kolekcja zawsze i wszędzie",
  często poza domem). `product_type` w PRD pozostaje `web-app`; charakter PWA i nacisk
  na mobile to wskazówka dla doboru stacku, nie decyzja PRD.
